...
const Patients = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingPatient) {
        await api.put(`/patients/${editingPatient.id}`, dataToSubmit);
  ...
  const handleStatusToggle = async (id: number, currentStatus: 'ACTIVE' | 'INACTIVE') => {
    try {
      ...
      console.log('Attempting to toggle patient status:', { id, currentStatus, targetStatus });
      const response = await api.patch(`/patients/${id}/status`, { 
        status: targetStatus
      });
      console.log('Status toggle response:', response);
      await fetchPatients();
      toast.success(`Patient ${targetStatus === 'ACTIVE' ? 'activated' : 'deactivated'} successfully`);
    } catch (error: any) {
  ...
  const handleBulkAction = async (action: 'activate' | 'deactivate' | 'delete') => {
    ...
    const confirmMessage = action === 'delete' 
      ...
      : `Are you sure you want to ${action} ${selectedPatients.length} patient(s)?`;
    ...
    try {
      const promises = selectedPatients.map(id => {
        if (action === 'delete') {
          ...
          return api.patch(`/patients/${id}/status`, { 
      ...
      toast.success(`Successfully ${action}d ${selectedPatients.length} patient(s)`);
    } catch (error: any) {
      console.error(`Failed to ${action} patients:`, error);
      toast.error(`Failed to ${action} patients`);
    }
  };

  const handleExport = () => {
    const csvContent = [
      ['Name', 'Email', 'Phone', 'Date of Birth', 'Gender', 'Status', 'Registration Date'],
