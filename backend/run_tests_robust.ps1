# HMS Functionality Test Script (Robust)
...
if (!$adminToken) { 
    Write-Error "Could not get Admin token. Aborting."
    exit 1 
}
...
# Use static future date to avoid parsing issues
$dateTimeStr = (Get-Date).AddDays(5).ToString("yyyy-MM-ddTHH:mm:00")
...
try {
    ...
    # Check if generic exception or WebException
    if ($_.Exception.Response.StatusCode -eq [System.Net.HttpStatusCode]::Conflict) {
        ...
        Write-Error "FAILURE: Unexpected error code: $($_.Exception.Message)"
    }
}
